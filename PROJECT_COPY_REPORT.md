...
> **清单审计**：2026-07-09 已按当前仓库代码更新 §10.3–10.9 勾选；完整阶段说明以 `VCA_IMPLEMENTATION_ROADMAP.md` 为准。未勾选项多为参考项目能力或刻意后置（在线曲库、完整 ModelScope 站、安装器）。
...
- [x] 创建任务
- [x] 串行队列执行
- [x] 人声分离（`song` 模式 UVR，失败可降级）
- [x] 去混响（可选，UVR 链路）
- [ ] F0 提取（推理引擎内部，无独立 UI）
- [x] 模型推理（RVC / So-VITS-SVC）
- [x] 混音（有伴奏 ffmpeg 混音）
- [ ] 作品状态轮询（前端需手动刷新）
- [x] 失败写日志
- [x] 作品可重试
...
- [x] 选择多个模型（`create_work` API；Create 页 UI 仍为单模型）
- [x] 每个模型独立参数（API + Editor 指派）
- [x] 导入本地 LRC（`lrc_parser` + `segment_builder`；创建页无 LRC 表单）
- [x] LRC 生成片段时间轴
- [ ] 歌词 / 音频整体偏移校正
- [x] 音频时长校验（拼接 end 推断）
- [x] 歌词 / 片段时间轴显示（Editor 片段表）
- [x] 逐句分配单个模型
- [ ] 批量模型分配（无专用批量 UI）
- [x] 未指派区间填充静音 / 原始人声占位（mute / original）
- [x] 每个模型整轨推理
- [x] 按分配片段裁切
- [x] 同源连续片段合并（拼接策略）
- [x] 换人处交叉淡化（片段边缘 fade）
...
- [x] 自动人声合并
- [ ] 手动人声合并
- [x] 自动 + 编辑器二次调整（时间轴编辑 + rerender）
- [ ] 全手动编辑（完整多轨工程）
- [ ] 单模型禁用人声合并类工作流
- [x] 多模型可进入编辑工程（`/editor/:id`）
...
- [ ] 读取远程清单
- [ ] 清单 magic / schema 校验
- [x] 下载模型（链接 / ZIP，`import_model_from_url` MVP）
- [x] 下载后导入本地模型（同上）
- [ ] 上传本地模型
- [ ] 自动生成 manifest
- [ ] 仓库名前缀规范
...
- [x] 歌词切分（片段 text + LRC）
- [ ] 片段人声分离
- [x] 局部重推理（`rerender_work`）
- [ ] 1 秒最短片段保护
- [ ] undo / redo
- [x] 导出 WAV（作品库导出 `final.wav`）
- [ ] 导出 MP3
- [ ] 导出 FLAC
