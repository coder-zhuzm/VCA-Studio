...
1. 启动桌面应用 → `/runtime` **本机摘要**应见 GPU 与推荐设备 **cuda**
2. **就绪清单**：检测/安装 **ffmpeg**（winget 安装或「检测并绑定」）→ 状态 `ready`
3. **就绪清单**：安装 **RVC（CUDA）** → 状态 `ready`（自动写入 `rvc_python`）
4. `/models` 导入 RVC `.pth` → `/create` 设备默认 **cuda** → 勾选立即开始 → `/works` 试听

UVR / So-VITS-SVC 仍手填路径（本页不提供一键安装）。日志：数据目录下 `runtime_install.log`。
